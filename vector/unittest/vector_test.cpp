...
template <typename T>
using JefLibVector = Vector<T, Allocator<T>, RandomAccessIterator<T>>;
...
TEST_F(VectorTest, EmplaceBack) {
  JefLibVector<S<int>> vec{{1, 2}, {3, 4}};
...
TEST_F(VectorTest, Iterators) {
  ...
  for (; vec_it != vec.rend() && vec3_it != vec3.rend(); ++vec_it, ++vec3_it) {
    res &= (*vec3_it == *vec_it);
  }
